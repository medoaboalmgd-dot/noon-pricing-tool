...
const db = {
  ...
  upsertProduct: (p) => sb("products", { method: "POST", prefer: "resolution=merge-duplicates,return=representation", body: JSON.stringify(p) }),
...
// ===================== IMPORT MODAL =====================
const ImportModal = ({ onClose, onImport, userName }) => {
  ...
  const doImport = async () => {
    if (!scrapeText.trim()) { alert("الصق بيانات الشيت أولاً"); return; }
    setLoading(true);
    setMsg("🔍 جاري معالجة البيانات...");
    const rows = parseSheet(scrapeText);
    const shipMap = {};
    if (shipText.trim()) {
      parseSheet(shipText).forEach(r => {
        const sk = (r.sku || r["كودالمنتج"] || "").toUpperCase();
        const sh = parseFloat(r.shipping || r["شحن"] || r["سعرالشحن"] || 0);
        if (sk) shipMap[sk] = sh;
      });
    }
    ...
    const products = rows.map(row => {
      const url = row.url || row.link || row["لينك"] || "";
      const sku = (row.sku || extractSKU(url) || "").toUpperCase();
      const egUrl = buildEgyptUrl(sku);
      const shipping = shipMap[sku] || 0;
      const uaePrice = parseFloat(row.currentprice || row.price || row["سعر"] || 0);
      const cost = uaePrice > 0 ? calcCost(uaePrice, aedRate, shipping) : null;
      const sellingPrice = cost ? calcSelling(cost) : null;
      return {
        id: `${sku || Math.random().toString(36).slice(2)}_${Date.now()}`,
        sku: sku || null,
        sku_type: skuType(sku),
        title: row.title || row["اسمالمنتج"] || "",
        brand: row.brand || row["براند"] || "",
        image: row.image || row["صورة"] || "",
        uae_url: url,
        egypt_url: egUrl,
    ...
    setMsg(`⬆️ جاري رفع ${products.length} منتج...`);
    try {
      await db.upsertProducts(products);
      setMsg(`✅ تم رفع ${products.length} منتج بنجاح`);
      setTimeout(() => { onImport(); onClose(); }, 800);
    } catch (e) {
  ...
  return (
    <div style={S.overlay}>
      <div style={{ ...S.modal, maxWidth: 600 }}>
        <div style={S.modalHead}>
          <span style={S.modalTitle}>📤 استيراد شيت السكراب</span>
          <button onClick={onClose} style={S.closeBtn}>✖</button>
        </div>
        <div style={S.tabRow}>
          {[["scrape","📋 شيت السكراب"],["ship","🚚 شيت الشحن (اختياري)"]].map(([id,lbl]) => (
            <button key={id} onClick={() => setTab(id)} style={{ ...S.tabBtn, ...(tab===id ? S.tabActive : {}) }}>{lbl}</button>
          ))}
        </div>
        {tab === "scrape" && (
          <div>
            <p style={S.hint}>الصق محتوى الشيت من Excel أو Google Sheets (Ctrl+A ثم Ctrl+C من الشيت)</p>
            <textarea value={scrapeText} onChange={e => setScrapeText(e.target.value)} style={S.textarea} rows={9}
              placeholder={"title\turl\timage\tbrand\tcurrentPrice\nProduct 1\thttps://noon.com/uae-en/product/N123/p/\thttps://img...\tSamsung\t499"} />
            <p style={S.hint}>الأعمدة المطلوبة: <code>url</code> — الباقي اختياري</p>
          </div>
        )}
        {tab === "ship" && (
          <div>
            <p style={S.hint}>شيت فيه عمودين: <code>sku</code> و <code>shipping</code> (بالجنيه)</p>
            <textarea value={shipText} onChange={e => setShipText(e.target.value)} style={S.textarea} rows={7}
              placeholder={"sku\tshipping\nN12345678901\t150\nZ98765432101\t200"} />
          </div>
        )}
        {msg && <div style={S.statusMsg}>{msg}</div>}
        <div style={{ display:"flex", gap:8, justifyContent:"flex-end", marginTop:12 }}>
          <button onClick={onClose} style={S.btnGhost}>إلغاء</button>
          <button onClick={doImport} disabled={loading} style={S.btnPrimary}>{loading ? "⏳ جاري..." : "✅ استيراد"}</button>
        </div>
      </div>
    </div>
  );
...
// ===================== SCRAPE EGYPT MODAL =====================
const ScrapeModal = ({ onClose, products, onDone, userName }) => {
  const [logs, setLogs] = useState([]);
  const [progress, setProgress] = useState(0);
  const [running, setRunning] = useState(false);
  const [done, setDone] = useState(false);

  const log = (msg) => setLogs(l => [...l, { msg, time: new Date().toLocaleTimeString("ar-EG") }]);
    ...
    for (let i = 0; i < toScrape.length; i++) {
      ...
      try {
        ...
        if (items.length > 0) {
          ...
          // Recalculate cost & selling with new data
          const aedSetting = await db.getSetting("aed_rate");
          const aedRate = aedSetting?.rate || 13.6;
          const cost = p.uae_price > 0 ? calcCost(p.uae_price, aedRate, p.shipping || 0) : p.cost;
          const sellingPrice = cost ? calcSelling(cost) : p.selling_price;
          ...
          await db.updateProduct(p.id, {
            ...
            cost, selling_price: sellingPrice,
            last_updated: today(),
          });
          log(`  ✅ سعر نون مصر: ${newPrice} ج.م | ${item.isBuyable ? "متاح ✅" : "غير متاح ❌"}`);
        } else {
  ...
  return (
    <div style={S.overlay}>
      <div style={{ ...S.modal, maxWidth: 540 }}>
        ...
        <p style={S.hint}>سيشتغل على <strong>{products.filter(p=>p.egypt_url).length}</strong> منتج عندهم لينك مصر</p>
        {!running && !done && (
        ...
        {(running || done) && (
          <>
            ...
            <div style={S.logBox} id="logbox">
              {logs.map((l,i) => (
                <div key={i} style={{ display:"flex", gap:8, marginBottom:3 }}>
                  <span style={{ color:"#475569", fontSize:10, minWidth:55 }}>{l.time}</span>
                  <span style={{ color: l.msg.includes("❌") ? "#f87171" : l.msg.includes("✅") ? "#4ade80" : l.msg.includes("⚠️") ? "#fbbf24" : "#94a3b8", fontSize:12 }}>{l.msg}</span>
                </div>
              ))}
            </div>
          </>
...
// ===================== DASHBOARD =====================
const Dashboard = ({ products, aedRate }) => {
...
// ===================== PRODUCT ROW =====================
const ProductRow = ({ p, aedRate, onShipChange, onDelete, onRefresh }) => {
...
export default function App() {
  ...
  return (
    <div style={S.app} dir="rtl">
      ...
      {/* ACTIONS */}
      <div style={S.actions}>
        <button onClick={() => setShowImport(true)} style={S.btnPrimary}>📤 استيراد شيت</button>
        <button onClick={() => setShowScrape(true)} style={{ ...S.btnPrimary, background:"#059669" }}>🇪🇬 سكراب نون مصر</button>
      ...
      {/* DASHBOARD */}
      {showDash && <Dashboard products={products} aedRate={aedRate} />}
      ...
      <div style={S.tableWrap}>
        {loading ? (
          ...
          <div style={S.empty}>
            {products.length === 0 ? "📭 مفيش منتجات لسه — اضغط «استيراد شيت» للبدء" : "🔍 مفيش نتائج للفلتر ده"}
          </div>
        ) : (
          <table style={S.table}>
            <thead>
            ...
            <tbody>
              {filtered.map(p => (
                <ProductRow key={p.id} p={p} aedRate={aedRate} onShipChange={handleShipChange} onDelete={handleDelete} onRefresh={loadData} />
              ))}
            </tbody>
          </table>
        )}
      ...
      {showImport && <ImportModal onClose={() => setShowImport(false)} onImport={loadData} userName={userName} />}
      {showScrape && <ScrapeModal onClose={() => setShowScrape(false)} products={products} onDone={loadData} userName={userName} />}
    </div>
  );
}
...
// ===================== STYLES =====================
const S = {
  app: { fontFamily:"'Segoe UI',Tahoma,Arial,sans-serif", minHeight:"100vh", background:"#f1f5f9", color:"#1e293b", direction:"rtl" },
  ...
  statusMsg: { padding:"8px 12px", background:"#f0fdf4", borderRadius:8, fontSize:13, color:"#059669", marginTop:8 },
  textarea: { width:"100%", padding:"8px 10px", border:"1px solid #d1d5db", borderRadius:8, fontSize:12, fontFamily:"monospace", direction:"ltr", boxSizing:"border-box", resize:"vertical" },
  tabRow: { display:"flex", gap:0, marginBottom:14, borderBottom:"1px solid #e2e8f0" },
  tabBtn: { padding:"8px 14px", border:"none", background:"none", cursor:"pointer", fontSize:13, color:"#64748b", borderBottom:"2px solid transparent", marginBottom:-1 },
  tabActive: { color:"#6366f1", borderBottomColor:"#6366f1", fontWeight:600 },
  ...
  logBox: { background:"#0f172a", borderRadius:8, padding:"10px 12px", maxHeight:200, overflowY:"auto", fontFamily:"monospace" },
};
